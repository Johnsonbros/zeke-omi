...
- **Place Intelligence System**: Comprehensive location-aware feature set for geo-contextual AI assistance.
  ...
  - **Place Lists**: Named collections of places for grouping (e.g., "Workout Spots", "Client Sites", "Favorite Restaurants"). Many-to-many with ordering. Full CRUD API at `/api/places/lists` and `/api/places/lists/{id}/places`. **Clickable List Detail View**: Clicking any list card navigates to a detail view where users can manage the list - add/remove places, delete the list, and see all places in the collection.
